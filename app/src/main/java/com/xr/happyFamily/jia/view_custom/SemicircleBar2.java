 ...
 */
public class SemicircleBar2 extends View {
    ...
    public SemicircleBar2(Context context) {
        this(context, null);
    }

    public SemicircleBar2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SemicircleBar2(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        }
    ...
    private void initAttrs(AttributeSet attrs, int defStyle) {
        ...
        mReachedColor = a.getColor(R.styleable.CircleSeekBar_wheel_reached_color, getColor(R.color.white));
...
//        mPointerPaint.setColor(getResources().getColor(R.color.color_black3));
        ...
        /**移动空调*/
        for (int i = 0; i < 72; i++) {//总共45个点  所以绘制45次  //绘制一圈的小黑点
            if (i > 27 && i <45) {
                ...
                canvas.drawRect(centerX - TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, getResources().getDisplayMetrics()),
                        getPaddingTop() + mUnreachedWidth + TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 10, getResources().getDisplayMetrics()),
...
//            }
            canvas.rotate(5, centerX, centerY);//360度  绘制72次   每次旋转5度
        }
//        if (mNumColor == 0) {
...
//        return Math.round(mMaxProcess * ((float) mCurAngle / 360));
    ...
    public interface OnSeekBarChangeListener {
        void onChanged(SemicircleBar2 seekbar, double curValue);
    }
}