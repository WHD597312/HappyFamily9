...
//            }
    ...
    public void addFallObjects(final List<FallObject> fallObject, final int num) {
        getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
            @Override
            public boolean onPreDraw() {
                getViewTreeObserver().removeOnPreDrawListener(this);
                for (int i = 0; i < num; i++) {
                    for (int j=0;j<fallObject.size();j++){
                        FallObject fallObject1 = fallObject.get(j);
                        FallObject newFallObject = new FallObject(fallObject1.builder,viewWidth,viewHeight);
                        fallObjects.add(newFallObject);
