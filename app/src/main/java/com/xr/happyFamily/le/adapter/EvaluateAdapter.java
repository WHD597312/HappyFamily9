...
public class EvaluateAdapter extends BaseAdapter {
    ...
    private int layout;
    private String[] color={"#ffecec","#efdaff","dafff1","#fff9d0","#f1ffda","#dadeff","#fddbff"};
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(layout== R.layout.item_xuyuan_xq){
            int i= random.nextInt(color.length);
            holder.evaluate_tv.setTextColor(Color.parseColor(color[i]));
        }
        final String ee = getItem(position).toString();
        holder.evaluate_tv.setText(ee);

                  if (clickTemp == position) {
                      holder.evaluate_tv.setBackgroundResource(R.drawable.bg_shape_stroke_green25);
                      holder.evaluate_tv.setTextColor(Color.parseColor("#4FBA72"));
          } else {

                      holder.evaluate_tv.setBackgroundResource(R.drawable.bg_shape_stroke_gray25);
