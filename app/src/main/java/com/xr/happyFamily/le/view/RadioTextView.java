...
public class RadioTextView extends View{
    ...
    public RadioTextView(Context context, AttributeSet attrs, int defStyle) {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            switch (attr)
            {
                case R.styleable.RaidoTextView_radioTitleText:
                    mTitleText = a.getString(attr);
                    break;
                case R.styleable.RaidoTextView_radioTitleTextColor:
                    // 默认颜色设置为黑色
                    mTitleTextColor = a.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.RaidoTextView_radioTitleTextSize:
                    // 默认设置为16sp，TypeValue也可以把sp转化为px
                    mTitleTextSize = a.getDimensionPixelSize(attr, (int) TypedValue.applyDimension(
                            TypedValue.COMPLEX_UNIT_SP, 16, getResources().getDisplayMetrics()));
                    break;
                case R.styleable.RaidoTextView_radioBackground:
                    //默认为白色
                    background = a.getColor(attr, Color.WHITE);
                    break;
                case R.styleable.RaidoTextView_radioCornerSize:
                    //默认圆角为0
    ...
    public void setmTitleText(String string){
        mTitleText=string;
    }

    public void setBackground(String string){
        background=Color.parseColor(string);
    }
}