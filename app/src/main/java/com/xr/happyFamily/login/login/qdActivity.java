            ...
            try {
                if (!Utils.isEmpty(result)) {
                    ...
                    if (code == 100) {
                        ...
                        String uuid = UUID.getUUID(qdActivity.this);
                        if (!TextUtils.isEmpty(uuid)) {
                            JSONObject login = new JSONObject();
                            login.put("userId", uuid);
                            String ss = login.toString();
                            if (mqService != null) {
                                boolean success2 = mqService.publish("p99/" + phone + "/login", 1, ss);
                                Log.i("LoginAsyncTask", "-->" + success2);
                            }
                        }
                    }
                }
...
//                            }
                            ...
                            for (int j = 0; j < roomDevices.length(); j++) {
                                ...
                                for (int k = 0; k < deviceList.length(); k++) {
                                    ...
                                    int heaterControl=device.getInt("typeSole");
                                    ...
                                    deviceChild.setHeaterControl(heaterControl);
...
//                            }
                            ...
                            int shareLength = deviceShareds.length();
                            if (shareLength > 0) {
                                for (int x = 0; x < shareLength; x++) {
